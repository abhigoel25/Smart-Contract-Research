...
@mcp.tool()
def getLoanTerm():
    """Get getLoanTerm from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getLoanTerm\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getLoanTerm().call(block_identifier='latest')
...
@mcp.tool()
def getContractDate(index):
    """Get getContractDate from the contract.
    
    Args:
          index: uint256
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getContractDate\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getContractDate(index).call(block_identifier='latest')
...
@mcp.tool()
def meetObligation(index):
    """Call meetObligation on the contract.
    
    Args:
          index: uint256
    """
    try:
        sys.stderr.write(f"[MCP] Calling meetObligation\n")
        sys.stderr.flush()
        ...
        try:
            txn = contract.functions.meetObligation(index).build_transaction({
...
@mcp.tool()
def payLoan():
    """Call payLoan on the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling payLoan\n")
        sys.stderr.flush()
        ...
        # Build transaction with timeout
        try:
            txn = contract.functions.payLoan().build_transaction({
                'from': account_address,
                'nonce': nonce,
                'gas': 2000000,
                'gasPrice': web3.eth.gas_price
...
@mcp.tool()
def payMonthly():
    """Call payMonthly on the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling payMonthly\n")
        sys.stderr.flush()
        ...
        # Build transaction with timeout
        try:
            txn = contract.functions.payMonthly().build_transaction({
                'from': account_address,
                'nonce': nonce,
                'gas': 2000000,
                'gasPrice': web3.eth.gas_price
