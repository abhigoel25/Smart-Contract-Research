...
1. **BlockChain_Venture_Ca.sol** - Solidity smart contract (40 lines)
...
### MCP Server
5. **BlockChain_Venture_Ca_mcp_server.py** - Custom MCP server (170 lines)
...
- `getInvestmentDetails()` - Read-only query
- `company()` - Read-only query
- `investor()` - Read-only query
- `investmentAmount()` - Read-only query
- `purchasePricePerShare()` - Read-only query
