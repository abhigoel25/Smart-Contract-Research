...
@mcp.tool()
def constructor():
    """Call constructor on the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling constructor\n")
        sys.stderr.flush()
        ...
        try:
            txn = contract.functions.constructor().build_transaction({
...
def getFinancialTerm(index):
    ...
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getFinancialTerm\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getFinancialTerm(index).call(block_identifier='latest')
...
def getDateDetail(index):
    """Get getDateDetail from the contract.
    
    Args:
          index: uint256
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getDateDetail\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getDateDetail(index).call(block_identifier='latest')
...
def executeObligation(index):
    ...
    try:
        ...
        # Build transaction with timeout
        try:
            txn = contract.functions.executeObligation(index).build_transaction({
                'from': account_address,
                'nonce': nonce,
                'gas': 2000000,
                'gasPrice': web3.eth.gas_price
