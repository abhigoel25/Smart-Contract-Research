...
@mcp.tool()
async def getDisclosingParty():
    """Get getDisclosingParty from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getDisclosingParty\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getDisclosingParty().call(block_identifier='latest')
...
@mcp.tool()
async def getReceivingParty():
    """Get getReceivingParty from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getReceivingParty\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getReceivingParty().call(block_identifier='latest')
...
async def getEndDate():
    """Get getEndDate from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getEndDate\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getEndDate().call(block_identifier='latest')
...
async def getObligation(index):
    ...
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getObligation\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getObligation(index).call(block_identifier='latest')
...
@mcp.tool()
async def getObligationCount():
    """Get getObligationCount from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getObligationCount\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getObligationCount().call(block_identifier='latest')
...
@mcp.tool()
async def getSpecialTerm(index):
    """Get getSpecialTerm from the contract.
    
    Args:
          index: uint256
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getSpecialTerm\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getSpecialTerm(index).call(block_identifier='latest')
...
@mcp.tool()
async def getSpecialTermCount():
    """Get getSpecialTermCount from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getSpecialTermCount\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getSpecialTermCount().call(block_identifier='latest')
...
@mcp.tool()
async def getTerminationCondition(index):
    """Get getTerminationCondition from the contract.
    
    Args:
          index: uint256
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getTerminationCondition\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getTerminationCondition(index).call(block_identifier='latest')
...
@mcp.tool()
async def getTerminationConditionCount():
    """Get getTerminationConditionCount from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getTerminationConditionCount\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getTerminationConditionCount().call(block_identifier='latest')
...
@mcp.tool()
async def createObligation(_party, _description, _penaltyForBreach):
    """Call createObligation on the contract.
    
    Args:
          _party: string
          _description: string
          _penaltyForBreach: string
    """
    try:
        sys.stderr.write(f"[MCP] Calling createObligation\n")
        sys.stderr.flush()
        ...
        try:
            txn = contract.functions.createObligation(_party, _description, _penaltyForBreach).build_transaction({
...
@mcp.tool()
async def addSpecialTerm(term):
    """Call addSpecialTerm on the contract.
    
    Args:
          term: string
    """
    try:
        sys.stderr.write(f"[MCP] Calling addSpecialTerm\n")
        sys.stderr.flush()
        ...
        # Build transaction with timeout
        try:
            txn = contract.functions.addSpecialTerm(term).build_transaction({
                'from': account_address,
                'nonce': nonce,
                'gas': 2000000,
                'gasPrice': web3.eth.gas_price
...
@mcp.tool()
async def addTerminationCondition(condition):
    """Call addTerminationCondition on the contract.
    
    Args:
          condition: string
    """
    try:
        sys.stderr.write(f"[MCP] Calling addTerminationCondition\n")
        sys.stderr.flush()
        ...
        # Build transaction with timeout
        try:
            txn = contract.functions.addTerminationCondition(condition).build_transaction({
                'from': account_address,
                'nonce': nonce,
                'gas': 2000000,
                'gasPrice': web3.eth.gas_price
