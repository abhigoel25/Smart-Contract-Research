...
1. **Capital_Fi_GreenTech_.sol** - Solidity smart contract (120 lines)
...
### MCP Server
5. **Capital_Fi_GreenTech__mcp_server.py** - Custom MCP server (142 lines)
...
- `constructor()` - State-changing transaction
- `getLender()` - Read-only query
- `getBorrower()` - Read-only query
- `getFinancialTerm()` - Read-only query
- `getDateDetail()` - Read-only query
- `executeObligation()` - State-changing transaction
