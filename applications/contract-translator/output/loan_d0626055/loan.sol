...
contract LoanAgreement {
    ...
    function getFinancialTerm(uint256 index) public view returns (uint256, string memory, string memory, string memory, uint256) {
    ...
    function getDate(uint256 index) public view returns (string memory, uint256, uint256) {
        if (index >= dates.length) {
            return ("", 0, 0);
        }
        Date memory dateEntry = dates[index];
        return (dateEntry.dateType, dateEntry.value, dateEntry.dayOfMonth);
    }

    function getObligation(uint256 index) public view returns (string memory, string memory, uint256, string memory) {
    ...
    function fulfillObligation(uint256 index) public {
        if (index < obligations.length) {
            Obligation memory obligation = obligations[index];
            emit ObligationFulfilled(msg.sender, obligation.description);
        }
    }
}