...
1. **BlockChain_Venture_Ca.sol** - Solidity smart contract (130 lines)
2. **BlockChain_Venture_Ca.abi.json** - Contract ABI (9 elements)
...
### MCP Server
5. **BlockChain_Venture_Ca_mcp_server.py** - Custom MCP server (161 lines)
...
- `getParty()` - Read-only query
- `getFinancialTerm()` - Read-only query
- `getDate()` - Read-only query
- `getAsset()` - Read-only query
- `getObligation()` - Read-only query
- `executeInvestment()` - State-changing transaction
