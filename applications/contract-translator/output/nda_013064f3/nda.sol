...
contract MutualNonDisclosureAgreement {
    ...
    function getEndDate() public view returns (string memory, uint) {
        return (endDate.value, endDate.dayOfMonth);
    }

    // Getters for obligation
    function getObligation() public view returns (string memory, string memory, string memory) {
        return (obligation.party, obligation.description, obligation.penaltyForBreach);
    }

    function getConditions() public view returns (string memory) {
        return conditions.exceptions;
    }

    // Example action function that checks and executes an obligation
    function executeObligation() public returns (string memory) {
        if (bytes(obligation.description).length == 0) {
            return "No obligation description provided.";
        } else {
            // Logic for executing the obligation can go here
