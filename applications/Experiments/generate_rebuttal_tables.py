...
-----------------------------
Load all experiment results and generate publication-ready tables
formatted for copy-paste into the KDD rebuttal document.

Run this AFTER all experiments complete:
  python generate_rebuttal_tables.py --results_dir ./results
...
from experiment_utils import load_results, compute_statistics, extract_scores
...
def build_ablation_table(results_dir: Path) -> str:
    ...
    pairwise = safe_load_json(str(results_dir / "ablation" / "pairwise_significance.json"))
    seed_stability = safe_load_json(str(results_dir / "ablation" / "seed_stability.json"))
    ...
    if summary:
        for key, (label, desc) in ABLATION_LABELS.items():
            ...
            score = f"{cs.get('mean', 'N/A'):.2f}" if isinstance(cs.get("mean"), float) else "N/A"
            std   = f"{cs.get('std', 'N/A'):.2f}"  if isinstance(cs.get("std"),  float) else "N/A"
            comp  = f"{cr*100:.1f}%"                if isinstance(cr, float) else "N/A"
            m1v   = f"{m1.get('mean', 'N/A'):.1f}" if isinstance(m1.get("mean"), float) else "N/A"
            m3v   = f"{m3.get('mean', 'N/A'):.1f}" if isinstance(m3.get("mean"), float) else "N/A"
            m4v   = f"{m4.get('mean', 'N/A'):.1f}" if isinstance(m4.get("mean"), float) else "N/A"
...
def build_gt_compilation_table(results_dir: Path) -> str:
    ...
    if summary:
        lines.append(
            f"  {'Generated contracts (paper reported)':<40} {'86.54%':>12}"
        )
        ...
        lines.append("-" * 65)
        lines.append(f"  {'Version bucket':<20} {'Success rate':>15} {'N contracts':>12}")
        lines.append("-" * 65)
        for bucket, stats in sorted(
            summary.get("by_version_bucket", {}).items()
        ):
            rate = stats.get("rate", 0)
            n    = stats.get("total", 0)
            lines.append(
                f"  {bucket:<20} {f'{rate*100:.1f}%':>15} {n:>12}"
            )
        lines.append("-" * 65)
        ...
        lines.append("  [RESULTS NOT YET AVAILABLE — run experiment_2_ground_truth_compilation.py]")
...
def build_temperature_table(results_dir: Path) -> str:
    ...
    if summary:
        ...
        for temp_str in temps:
            ...
            comp  = f"{cr*100:.1f}%"               if isinstance(cr, float) else "N/A"
            lines.append(
                f"  {temp_str:>12} {score:>10} {std:>6} {comp:>9} {n:>5}"
            )
...
def build_slither_table(results_dir: Path) -> str:
    summary = safe_load_json(
        str(results_dir / "slither" / "slither_summary.json")
    )
    ...
    lines.append("=" * 65)
    lines.append(
        f"  {'Metric':<40} {'Pre-refine':>12} {'Post-refine':>12}"
    )
    lines.append("-" * 65)
    ...
    if summary:
        ...
        lines.append(
            f"  {'(paper-reported LLM critical reduction)':<40} {'88.2%':>25}"
        )
    else:
...
def build_debiased_table(results_dir: Path) -> str:
    summary = safe_load_json(
        str(results_dir / "debiased" / "debiased_summary.json")
    )
    ...
    if summary:
        ...
        lines.append("  [RESULTS NOT YET AVAILABLE — run experiment_5_debiased_metric.py]")
...
def build_contract_type_table(results_dir: Path) -> str:
    ...
    lines = []
    lines.append("TABLE 6: CROSS-CONTRACT-TYPE GENERALIZATION (Addressing Reviewer J5uF)")
    lines.append("=" * 90)
    lines.append(
    ...
    if summary:
        ...
        for cat in sorted(cats):
            ...
            n   = s.get("n", 0)
            avg = f"{s['composite_mean']:.2f}" if s.get("composite_mean") is not None else "N/A"
            std = f"{s['composite_std']:.2f}"  if s.get("composite_std")  is not None else "N/A"
            cr  = f"{s['compilation_rate']*100:.1f}%" if s.get("compilation_rate") is not None else "N/A"
        ...
        lines.append("  [RESULTS NOT YET AVAILABLE — run experiment_6_contract_type.py]")
...
def build_novelty_table() -> str:
    rows = [
        ("Security gate with severity threshold",  "❌ No",  "✅ Yes — blocks downstream if severity ≥ medium"),
        ("Deterministic composite score",          "❌ No",  "✅ Yes — recomputed, not model-generated"),
        ("FSM-grounded evaluation rubric",         "❌ No",  "✅ Yes — 5-dim rubric tied to FSM state correctness"),
        ("Paired ground-truth comparison",         "❌ No",  "✅ Yes — per-metric deltas vs expert implementations"),
        ("Solc compilation as hard gate",          "❌ No",  "✅ Yes — integrated via SolidityCompilationChecker"),
        ("ABI + MCP server generation",            "❌ No",  "✅ Yes — full deployment-adjacent workflow"),
        ("Separate Generator ↔ Refiner agents",   "Optional","✅ Yes — prevents optimistic self-evaluation bias"),
    ]

    lines = []
    lines.append("TABLE 6: PIPELINE NOVELTY vs. VANILLA CREWAI (Addressing Reviewer inVW Q2)")
    lines.append("=" * 90)
    lines.append(
        f"  {'Feature':<45} {'Vanilla CrewAI':>14}  {'This Work':>28}"
    )
    lines.append("-" * 90)
...
def generate_all_tables(results_dir: Path, output_dir: Path) -> None:
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("KDD 2026 REBUTTAL — SUPPORTING TABLES\n")
...
def main():
    parser = argparse.ArgumentParser(description="Generate rebuttal tables from experiment results")
