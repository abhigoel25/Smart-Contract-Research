...
Handles:
- Dataset loading and sampling from FSM-SCG requirement_code.jsonl
- Contract processing via the streaming translator pipeline
- Score extraction from quality evaluation results
- Result persistence and statistics computation
...
# Experiments/ is two levels inside the workspace root:
#   <workspace>/applications/Experiments/
# parents[0] = Experiments/, parents[1] = applications/, parents[2] = workspace root
REPO_ROOT = Path(__file__).resolve().parents[2]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

# Load .env from applications/ so OPENAI_API_KEY etc. are available
...
def process_single_contract(
    ...
    """
    # Write requirement to a temp text file so the streaming method can read it
    ...
    # ── Also compile the ground-truth code so we can compare ────────────────
...
def save_results(results: List[Dict], output_path: str) -> None:
    """
    Append-safe JSONL writer.  Each call overwrites the file.
    """
...
def get_raw_scores(results: List[Dict]) -> Dict[str, Any]:
    ...
    use in paired statistical tests across conditions.

    Returns a dict:
        {
            "indices":   [<contract index>, ...],
            "composite": [float, ...],
            "m1": [float, ...], "m2": [...], "m3": [...], "m4": [...], "m5": [...],
        }
    ...
    for r in results:
        ...
        if s["composite"] == 0.0 and s["m1"] == 0.0:
            # skip completely null evaluations
            continue
...
def compute_pairwise_significance(
    ...
    """
    Compute paired statistical tests between two experimental conditions.

    Aligns results by contract index so each pair tests the *same* contract
    processed under two different conditions.

    Tests:
      - Paired t-test (parametric, assumes normality, good for N > 30)
      - Wilcoxon signed-rank test (non-parametric, robust for any distribution)
      - Cohen's d effect size (mean_diff / std_diff of paired differences)

    Returns a dict keyed by metric name, each with:
        n_pairs, mean_a, mean_b, mean_diff, cohens_d, effect_size_label,
        t_stat, p_value_t, w_stat, p_value_w, stars (for p_value_t)

    Returns {"error": ..., "n_pairs": 0} if insufficient data or scipy missing.
    """
    if metrics is None:
...
def classify_error_modes(quality_evaluation: Optional[Dict]) -> Dict[str, bool]:
    """
    Map a quality evaluation result to structured error-mode categories.

    This addresses the [How are errors assessed?] placeholder cited by
    Reviewers J5uF and ybKn.  Categories are mutually non-exclusive — a
    single contract can exhibit multiple failure modes simultaneously.

    Returns:
        Dict with boolean flags for each error mode:
          logic_omission       — M1 < 60: required functions missing or empty
          state_transition_err — M3 < 60: state machine transitions incorrect
          access_control_fail  — M1 impl_quality low + M4 < 70
          economic_logic_err   — M4 financial_logic subscore missing
          naming_deviation     — M2 < 70 (potential naming mismatch, not error)
          temporal_logic_miss  — M4 temporal subscore = 0 when dates present
          code_quality_low     — M5 < 50 (placeholders / no error messages)
          fully_correct        — composite >= 80 across all metrics
    """
    if not quality_evaluation:
        return {
            "logic_omission": True,
...
def compute_statistics(results: List[Dict]) -> Dict:
    ...
    Returns:
        Dict with mean/std for composite score and each metric (both generated
        and ground-truth where available), compilation rates, and error rates.
    """
