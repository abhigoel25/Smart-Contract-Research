...
def getObligation():
    ...
    try:
        ...
        try:
            # Create a call with explicit timeout
            result = contract.functions.getObligation().call(block_identifier='latest')
...
def executeObligation():
    """Call executeObligation on the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling executeObligation\n")
        sys.stderr.flush()
        ...
        try:
            txn = contract.functions.executeObligation().build_transaction({
