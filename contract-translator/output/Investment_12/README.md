...
1. **BlockChain_Venture_Ca.sol** - Solidity smart contract (177 lines)
...
### MCP Server
5. **BlockChain_Venture_Ca_mcp_server.py** - Custom MCP server (188 lines)
...
- `getSeriesAStock()` - Read-only query
- `getRedemptionObligation()` - Read-only query
- `triggerInvestment()` - State-changing transaction
- `redeemShares()` - State-changing transaction
- `parseDate()` - Read-only query
