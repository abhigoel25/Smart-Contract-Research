...
@mcp.tool()
def getObligations():
    """Get getObligations from the contract.
    
    Returns: Contract data
    """
    try:
        sys.stderr.write(f"[MCP] Calling view function getObligations\n")
        sys.stderr.flush()
        
        # CRITICAL FIX: Add explicit timeout and error handling
        try:
            # Create a call with explicit timeout
            result = contract.functions.getObligations().call(block_identifier='latest')
...
def fulfillObligation(obligationIndex):
    ...
    try:
        ...
        try:
            txn = contract.functions.fulfillObligation(obligationIndex).build_transaction({
...
if __name__ == "__main__":
    sys.stderr.write("[MCP] Starting mcp server...\n")
    sys.stderr.flush()
    
    try:
        import asyncio
        sys.stderr.write("[MCP] Using asyncio.run() to start server\n")
        sys.stderr.flush()
        asyncio.run(mcp.run())
    except Exception as e:
