...
## Contract Summary
- **Type**: service
- **Parties**: CloudTech Solutions, Inc., MediCare Network, Inc.
...
1. **CloudTech__MediCare_N.sol** - Solidity smart contract (88 lines)
2. **CloudTech__MediCare_N.abi.json** - Contract ABI (11 elements)
...
### MCP Server
5. **CloudTech__MediCare_N_mcp_server.py** - Custom MCP server (162 lines)
...
### 4. Run MCP Server
```bash
python CloudTech__MediCare_N_mcp_server.py
```
...
- `getTerm()` - Read-only query
- `paySetupFee()` - Payable transaction
- `payMonthlyFee()` - Payable transaction
