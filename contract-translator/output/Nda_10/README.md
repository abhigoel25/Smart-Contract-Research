...
1. **InnovateTe_FutureInve.sol** - Solidity smart contract (38 lines)
2. **InnovateTe_FutureInve.abi.json** - Contract ABI (8 elements)
...
### MCP Server
5. **InnovateTe_FutureInve_mcp_server.py** - Custom MCP server (105 lines)
...
- `confirmConfidentiality()` - Read-only query
- `reportBreach()` - State-changing transaction
- `calculatePenalty()` - Read-only query
- `getConfidentialParties()` - Read-only query
- `getTermDays()` - Read-only query
